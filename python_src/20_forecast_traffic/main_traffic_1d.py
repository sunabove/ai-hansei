...
class TrafficForeCast :
    ...
    def trainModel(self) :
        ...
        model.add( Dense( x_dim , input_shape=(None,x_dim) ) )

        x_layer_cnt = 2*y_dim ; y_layer_cnt = 4

        for i in range( x_layer_cnt ) :
            model.add( Dense( x_dim ) )
        pass

        for i in range( y_layer_cnt ):
    ...
    # test model
    def testModel(self, model, ds_test ):
        print( LINE )
        ...
        for i in range( len( x_list ) ) :
            ...
            for y in yList :  
                ...
                for yv in y : 
                    x_idx = x_dim - len(yList)*len(y) + (r*y_dim + c)
                    x = x_input[0][0][ x_idx ]
                    
                    if x == -999.0 or x == -999 :
                        x_input[0][0][ x_idx ] = yv

                        debug and log.info( "x[%d] = %s -> %s" % ( x_idx, x, yv ) )
                    elif r == len(yList) -1 :
                        y[ c ] = x
                        debug and log.info( "y[%d] = %s -> %s" % ( c, y[c], x ) )
                    pass 
...
if __name__ == '__main__':
    ...
    if "test" == mode :
        ...
        trafficForeCast.testModel( model, ds[ phase] )
    else : 
        trafficForeCast.trainModel()
    pass
